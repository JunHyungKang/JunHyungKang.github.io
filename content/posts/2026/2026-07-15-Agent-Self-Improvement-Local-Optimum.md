...
토큰 수는 마지막 7일 구간의 로컬 Codex session log에서 메인 에이전트와 subagent의 `token_count` 누적 증가분을 합산한 값이다. 총 input은 약 `28.99억`, 그중 cached input은 약 `27.91억`으로 input의 약 `96.3%`였고, uncached input은 약 `1.08억`, output은 약 `919만` tokens였다. 따라서 이 숫자는 API 청구량이나 비용이 아니라, 캐시를 포함해 하네스가 **처리한 전체 token usage**를 뜻한다.

29억이라는 숫자 자체는 성과 지표가 아니다. 이 프로젝트에서는 오히려 같은 context와 검증 절차가 반복해서 소비된 정도를 보여주는 **시스템 부하와 탐색 효율성의 진단 지표**에 가깝다. 점수 대부분이 공개 reference에서 왔기 때문에 `tokens per leaderboard point` 같은 비율을 계산하는 것도 인과적으로 타당하지 않다.
...
여기서 self-improvement는 모델 weight를 업데이트했다는 뜻이 아니다. 에이전트가 실행 증거를 바탕으로 prompts, skills, memory, scripts, validator, ranking policy 같은 **외부 하네스를 지속적으로 수정한 것**을 뜻한다. 최근 연구에서는 이를 `harness self-evolution` 또는 `agentic harness engineering`으로 구분한다. [Agentic Harness Engineering](https://arxiv.org/abs/2604.25850)은 하네스의 각 변경을 예측과 다음 평가 결과가 연결된 falsifiable contract로 다룬다.

또한 local optimum은 수학적으로 stationary point를 증명했다는 표현이 아니다. terminal objective인 순위 격차는 거의 줄지 않는데 proxy metric과 작업 처리량만 계속 좋아진 상태를 설명하는 **운영적 진단**이다.
...
구조화된 진행 파일과 clean handoff로 context window 사이를 연결하는 방식은 장기 실행 agent harness의 일반적인 설계와도 맞는다. Anthropic의 [Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) 역시 매 session이 incremental progress와 다음 session을 위한 명시적 상태를 남기도록 구성한다.
...
하지만 artifact는 trajectory가 남았다는 뜻이지 목표를 달성했다는 뜻이 아니다. Agent eval 관점에서는 trace와 outcome을 분리해야 한다. Anthropic의 [Demystifying evals for AI agents](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)도 transcript에 기록된 행동과 environment의 최종 상태를 별도 평가 대상으로 정의한다. 이 프로젝트는 한동안 artifact 생성량을 outcome 개선의 대리 지표로 잘못 사용했다.
...
| 하네스 층 | 최적화한 신호 | 본래 역할 | 실제로 생긴 편향 |
|---|---|---|---|
| Goal | Public rank 1 | Terminal objective | 장기 목표라 매 loop의 선택에는 약하게 작용 |
| Ranking | Official cost delta, 성공 확률 | Score proxy | 작고 확실한 개선을 과대평가 |
| Validator | Exactness, compatibility, root safety | Constraint | 고분산 탐색을 조기에 제거 |
| Progress contract | Loop당 artifact 1개 | Throughput proxy | 목표 기여보다 산출물 생성에 보상 |

문제는 각 proxy가 틀렸다는 것이 아니다. 모두 필요한 신호였다. 실패는 이 신호들을 terminal objective와 다시 보정하는 **outer-loop evaluation**이 없었다는 데 있었다.
...
다만 subagent 수를 늘리는 것과 탐색 품질을 높이는 것은 같지 않다. Anthropic의 [multi-agent research system 회고](https://www.anthropic.com/engineering/multi-agent-research-system)는 병렬 탐색에서 token scaling의 효과를 보고하면서도, 의존성이 높은 coding task에서는 coordination cost 때문에 multi-agent가 항상 적합하지 않다고 설명한다. 우리도 독립적인 분석에는 병렬화가 유효했지만, 같은 graph를 여러 agent가 중복 검토할 때는 token만 늘어났다.
...
여기서 SLA는 외부 고객에게 약속하는 service-level agreement가 아니라, exploit의 긴급성에 밀려 explorer queue가 사라지지 않도록 보장하는 **내부 budget contract**를 뜻한다.

다시 한다면 단순히 `60% exploit / 40% exploration`처럼 고정 비율을 Goal에 적지는 않을 것이다. 이 비율은 설명을 위한 예시일 뿐 실험에서 도출된 값이 아니다. 대신 후보별 expected score gain, uncertainty, time-to-evidence, family reuse, portfolio diversity를 기록하고, 최소 exploration floor와 task별 wall-clock/token cap을 scheduler가 강제하도록 만들 것이다. 매일 실제 score movement를 기준으로 비율을 다시 조정해야 한다.
...
이 글의 local optimum 진단은 실험 결과에 근거하지만 causal proof는 아니다.

- harness revision별 randomized ablation을 수행하지 않았다.
- 각 revision을 비교할 frozen held-out task suite와 replay set이 없었다.
- public leaderboard 변화에는 공개 reference 교체와 task graft가 섞여 있었다.
- exploration 비중을 달리한 병렬 run이 없어 다른 allocation policy의 결과를 직접 비교할 수 없다.

따라서 “validator를 강화해서 순위가 낮아졌다”거나 “exploration을 40%로 배정했으면 1등했다”고 결론 내릴 수는 없다. 증거가 지지하는 주장은 더 좁다. **하네스의 proxy metric은 반복해서 개선됐지만 terminal objective의 개선 속도는 충분하지 않았고, 이를 조기에 감지할 독립된 outer-loop eval이 없었다.**

다시 측정한다면 모든 harness edit에 `예상 효과 → frozen validation → held-out/OOD 결과 → 이전 실패 replay → token/cost 변화`를 연결할 것이다. [SEAGym](https://arxiv.org/abs/2606.17546)은 잦은 harness update가 held-out 성능을 높이지 않거나, 중간에 좋아진 snapshot이 이후 무너질 수 있음을 보여주며 이런 분리를 정식 evaluation protocol로 제안한다.
