...
contentType: "운영 모델"
evidence: "2026년 Microsoft, OpenAI, AWS의 Agent 운영 모델 자료를 대조하고 중앙 AX팀과 업무조직의 소유권 경계를 정리했습니다."
tags:
  - AX
  - AI Agent
  - Operating Model
...
예를 들어 구매 Agent가 발주 규정을 읽고 ERP에 주문을 만든다고 해보자. 첫 구현이 끝난 뒤에는 금액별 승인선이 바뀐다. 특정 공급사 예외가 추가되고 ERP 필드와 권한 정책도 달라진다. 주문이 실패했을 때 누가 재처리할지, 이미 생성된 주문을 어떻게 취소할지도 정해야 한다.
...
내가 중앙 AX팀에 두는 것은 Agent identity와 최소 권한, 승인된 tool·connector의 등록 방식, 실행 환경과 비밀정보 관리다. 공통 trace 형식과 평가·비용 관측, Agent registry와 배포 기준도 중앙에서 만든다. 중단과 원복 경로 역시 업무팀마다 처음부터 만들게 하지 않는다.
...
구매 Agent의 `create_order` tool을 예로 들면 중앙팀은 workload identity, credential 전달, 감사 로그 형식을 표준화할 수 있다. 중복 주문을 막는 멱등성 기준과 승인선, 이미 생성된 주문의 취소 규칙은 구매 시스템을 소유한 팀이 정해야 한다. 이 의미까지 중앙 AX팀이 떠안으면 ERP 정책이 바뀔 때마다 다시 중앙 대기열을 거치게 된다.
...
업무 시스템에 쓰기 작업을 수행한다면 승인 경계와 원복, 멱등성, 감사 로그를 공통 배포 기준에 넣는다. 업무 규칙과 예외 처리는 도메인이 맡되, 중앙 플랫폼이 기술적인 안전 조건을 강제한다.
