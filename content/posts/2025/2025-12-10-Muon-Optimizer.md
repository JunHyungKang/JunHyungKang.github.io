---
title: "딥러닝 왕좌에 도전장을 던진 Muon 옵티마이저: AdamW를 넘어 Kimi K2가 선택한 혁신 (초안)"
date: "2025-12-10"
...
지난 10년간 대규모 언어 모델(LLM) 학습 분야는 **아담(Adam)** 옵티마이저의 지배 아래 있었다 해도 과언이 아닙니다. 아담과 그 변형인 **AdamW**는 탁월한 안정성과 성능으로 업계 표준으로 자리 잡았죠.

하지만 모델의 규모가 수천억 개를 넘어 1조(Trillion) 개의 파라미터로 확장되면서, 기존 아담의 한계가 명확하게 드러나기 시작했습니다.
...
이러한 상황 속에서 **Kimi K2**와 같은 1조 파라미터 규모의 초거대 모델이 등장하며, 그들이 사용한 **뮤온클립(MuonClip)** 옵티마이저가 새로운 혁신의 물결을 일으키고 있습니다.

---

### 배경: 적응형 모멘트 추정(Adaptive Moment Estimation)의 시대

딥러닝 모델을 학습시킨다는 것은 결국 손실 함수(Loss Function)의 가장 낮은 지점을 찾아 산을 내려가는 과정과 같습니다. 이 과정을 이끄는 네비게이터가 바로 **옵티마이저(Optimizer)**입니다.

#### 1. SGD(확률적 경사 하강법)의 한계
초기의 **SGD**는 현재 위치에서 기울기(Gradient)가 가장 가파른 방향으로만 한 발자국씩 내딛는 단순한 방식이었습니다. 하지만 이 방식은 지그재그로 진동하거나, 평탄한 곳(Plateau)에 갇히면 빠져나오지 못하는 문제가 있었습니다. 이를 보완하기 위해 이전의 이동 방향(관성)을 반영하는 **모멘텀(Momentum)** 기법이 도입되었죠.

#### 2. Adam의 등장과 표준화
2014년 등장한 **Adam(Adaptive Moment Estimation)**은 혁명이었습니다. Adam은 두 가지 핵심 정보를 기억합니다:
*   **1차 모멘텀 ($m_t$):** 기울기의 평균 (방향)
*   **2차 모멘텀 ($v_t$):** 기울기 제곱의 평균 (보폭의 크기 조절)

Adam은 각 파라미터마다 학습률을 다르게 적용(Adaptive Learning Rate)하여, 학습이 더딘 파라미터는 과감하게, 이미 많이 변한 파라미터는 신중하게 업데이트합니다. 여기에 가중치 감소(Weight Decay) 로직을 수정한 **AdamW**가 나오면서, 이것은 LLM 학습의 '사실상의 표준(De Facto Standard)'이 되었습니다.

#### 3. 거대 모델 시대의 딜레마
하지만 모델이 커지면서 두 가지 심각한 문제가 발생했습니다.
첫째는 **메모리**입니다. Adam은 파라미터 하나당 $m_t, v_t$ 두 개의 상태를 저장해야 하므로, 1조 개의 파라미터 모델이라면 **2조 개의 추가 변수**를 위한 메모리가 필요합니다.
둘째는 **구조적 무시**입니다. Adam은 모든 파라미터를 1차원 벡터로 취급합니다. $1000 \times 1000$ 행렬이 가진 '2차원적 상관관계'나 '구조적 특징'은 무시된 채, 100만 개의 점들이 각각 따로 노는 것처럼 업데이트됩니다. 이것이 바로 우리가 이야기할 **Muon**이 등장하게 된 배경입니다.
