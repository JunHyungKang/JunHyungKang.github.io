...
*   **메모리 비효율성:** 아담은 각 파라미터마다 1차 모멘텀과 2차 모멘텀이라는 **두 개의 상태 변수를 추가로 저장**해야 하므로, 메모리 사용량이 모델 파라미터 자체의 **두 배**에 달합니다.
    ...
    | 항목 | Inference (추론) | Training (Adam 학습) |
    | :--- | :--- | :--- |
    | **모델 가중치 (Weights)** | 1 | 1 |
    | **그래디언트 (Gradients)** | 0 | 1 |
    | **옵티마이저 상태 (States)** | 0 | 2 (Momentum, Variance) |
    | **총 메모리 요구량** | **1x** | **4x** |

*   **저차원 업데이트:** 아담을 포함한 기존 옵티마이저들은 그래디언트 정보에 의존하여 업데이트합니다. 일반적으로 신경망의 그래디언트는 **상위 몇 개의 주성분(Principal Components)이 지배적인 특징**을 가집니다. 즉, 그래디언트의 에너지가 소수의 특정 방향에 쏠려 있다는 의미입니다. 이로 인해 파라미터 행렬이 아무리 거대해도, 실제 업데이트는 이 소수의 '지배적인 방향'으로만 주로 일어나게 되어, 나머지 수많은 차원들이 가진 잠재력을 충분히 활용하지 못하는 '저차원 행렬(low-rank matrix)' 업데이트와 같은 현상이 발생합니다.

Kimi K2 테크니컬 리포트를 읽다가 **뮤온클립(MuonClip)**이라는 생소한 용어를 발견했습니다. 처음 듣는 옵티마이저라 자료를 찾아보니, 이는 곧 **뮤온(Muon)**이라는 혁신적인 옵티마이저에 기반하고 있음을 알게 되었습니다. 그래서 이번 글에서는 뮤온클립을 이해하기 위한 첫걸음으로, 먼저 **뮤온 옵티마이저**가 무엇인지, 그리고 왜 1조 파라미터 모델들이 이것을 선택했는지 집중적으로 살펴보려 합니다.
